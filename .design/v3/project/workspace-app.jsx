...
function Workspace({ tweaks, forceNarrow }) {
  ...
  const onPickTool = (part, partId) => { setActivePart(part); setActivePartId(partId); setActiveDiff(null); setActiveDiffId(null); if (effectiveNarrow) setSheetOpen(true); };
  const onPickDiff = (part, partId) => { setActiveDiff(part); setActiveDiffId(partId); setActivePart(null); setActivePartId(null); if (effectiveNarrow) setSheetOpen(true); };
  const onCloseInspector = () => { setActivePart(null); setActivePartId(null); setActiveDiff(null); setActiveDiffId(null); };

  // j/k navigation: move focus AND clear any drill-in so the inspector
  // follows the focused message. Tool/diff drill-in is restored by clicking.
  const stepFocus = (delta) => {
    setActivePart(null); setActivePartId(null);
    setActiveDiff(null); setActiveDiffId(null);
    setFocusedMsg(i => Math.max(0, Math.min(WS_MESSAGES.length - 1, i + delta)));
  };

  const jumpToTurnId = (id) => {
    const idx = WS_MESSAGES.findIndex(m => m.id === id);
    if (idx >= 0) { setActivePart(null); setActivePartId(null); setActiveDiff(null); setActiveDiffId(null); setFocusedMsg(idx); }
  };

  const jumpBack = () => {
    const id = activePartId || activeDiffId;
    if (id && transcriptRef.current) {
    ...
    // Otherwise jump to the focused message
  ...
  React.useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === "/") {
        ...
        e.preventDefault(); setReportOpen(o => !o);
    ...
    <div className="cc-mock" style={{ display:"flex", height:"100vh", width:"100vw", overflow:"hidden", background:"var(--bg)" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0, background:"var(--bg)" }}>
        ...
        <div style={{ padding:"12px 22px", borderBottom:"1px solid var(--border)", background:"var(--surface)",
          ...
          <button onClick={() => setReportOpen(true)} className="cc-icon-btn"
            title="Session report — tokens, files, spikes (r)"
            style={{ padding:"6px 10px", border:"1px solid var(--border)", background:"var(--surface)", color:"var(--text)" }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:14, padding:"6px 22px", borderTop:"1px solid var(--border)",
          ...
          <span><kbd style={kbdStyle}>r</kbd> report</span>
      ...
      {!effectiveNarrow && rightOpen && (
        <div style={{ width:rightW, flex:"0 0 auto" }}>
          <RightRail activePart={activePart} activeDiff={activeDiff}
            focusedMessage={WS_MESSAGES[focusedMsg]} messages={WS_MESSAGES}
            onJumpBack={jumpBack} onJumpToPart={onPickTool} onJumpToTurn={jumpToTurnId}
      ...
      <SessionReport open={reportOpen} onClose={() => setReportOpen(false)} session={active} onJumpToTurn={() => setReportOpen(false)}/>
    </div>
  );
}
